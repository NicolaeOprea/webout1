...
function AdminLayout() {
  ...
  return <div className="min-h-screen bg-cream text-stone lg:grid lg:grid-cols-[290px_1fr]"><aside className={`fixed inset-y-0 left-0 z-40 w-[290px] border-r border-stone/10 bg-stone p-5 text-cream transition-transform duration-300 lg:static lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}><div className="flex items-center justify-between"><BrandLogo to="/" label="Sapore Admin" compact className="text-cream" /><button type="button" className="rounded-full p-2 hover:bg-white/10 lg:hidden" onClick={() => setIsSidebarOpen(false)} aria-label="Menu schließen"><X size={20} /></button></div><div className="mt-8 flex items-center gap-3 rounded-2xl bg-white/10 p-4"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-terracotta font-semibold">{initials}</span><div className="min-w-0"><div className="truncate font-semibold">{currentUser?.name || "Restaurant Admin"}</div><div className="truncate text-sm text-cream/65">{currentUser?.email}</div></div></div><nav className="mt-8 space-y-2">{adminNavigation.map((item) => { const Icon = item.icon; return <NavLink key={item.to} to={item.to} end={item.end} onClick={() => setIsSidebarOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-2xl px-4 py-3 transition ${isActive ? "bg-cream text-stone" : "text-cream/75 hover:bg-white/10 hover:text-cream"}`}><Icon size={19} /><span><span className="block font-medium">{item.label}</span><small className="block text-xs opacity-70">{item.caption}</small></span></NavLink>; })}</nav></aside><div className="min-w-0"><header className="sticky top-0 z-30 border-b border-stone/10 bg-cream/90 px-4 py-4 backdrop-blur md:px-8"><div className="flex items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><button type="button" className="rounded-full border border-stone/10 bg-white p-2 lg:hidden" onClick={() => setIsSidebarOpen(true)} aria-label="Menu öffnen"><Menu size={20} /></button><div className="min-w-0"><span className="text-xs font-semibold uppercase tracking-[0.18em] text-terracotta md:tracking-[0.24em]">Restaurant Verwaltung</span><h1 className="truncate font-serif text-xl text-stone md:text-2xl">Sapore Mediterraneo</h1></div></div><div className="flex shrink-0 items-center gap-2"><Link to="/" className="rounded-full border border-stone/10 bg-white px-3 py-2 text-sm font-semibold text-stone transition hover:border-terracotta hover:text-terracotta">Home</Link><button type="button" className="inline-flex items-center gap-2 rounded-full border border-stone/10 bg-white px-3 py-2 text-sm font-semibold text-stone transition hover:border-terracotta hover:text-terracotta md:px-4" onClick={() => logout()}><LogOut size={16} /><span className="hidden sm:inline">Logout</span></button></div></div></header><main className="px-4 py-8 md:px-8"><Outlet /></main></div></div>;
}
